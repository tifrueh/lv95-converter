...
to WGS84 and vice versa. The formulae used are the ones provided by
...
0. Install the build system [meson](https://mesonbuild.com) and the command-line
   parsing library [cli11](https://github.com/CLIUtils/CLI11).

1. Download the latest tarball and extract it.

2. Execute `meson setup build` from inside the source directory to set up the
...
4. Execute `meson install -C build` from inside the source directory to install
   the executable and the man page.

## Additional Information

Please refer to the manual page for any additional information on the program.

If you want to view the man page without installing it, clone the repository and
navigate your terminal to it. You can then use `man` to display the manual page
contained in the Documentation directory of this repository. This could be done,
for example, like so:
